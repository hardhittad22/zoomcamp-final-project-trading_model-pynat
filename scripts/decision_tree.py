...
def train_decision_tree_model():
    ...
    for depth in range(1, 21):
        ...
        clf, train_columns = fit_decision_tree(X=X_train_valid, y=y_train_valid, max_depth=depth)
        ...
        if precision_test >= best_precision:
            ...
            best_depth = depth
        tree_rules = export_text(clf, feature_names=list(X_train_valid.columns), max_depth=3)
        print(tree_rules)
        print('------------------------------')
    ...
    precision_df = pd.DataFrame(list(precision_by_depth.items()), columns=['max_depth', 'precision_score'])
    precision_df['precision_score'] = precision_df['precision_score'] * 100.0  # Convert to percentage
