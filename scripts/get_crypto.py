...
def get_coins():
    ...
    coins = [
        ...
        'DOGEUSDT', 'ADAUSDT', 'TRXUSDT', 'ARBUSDT', 'SHIBUSDT',
        'WBTCUSDT', 'DOTUSDT', 'LINKUSDT', 'BCHUSDT', 'UNIUSDT',
        'NEARUSDT', 'LTCUSDT', 'FILUSDT', 'BEAMUSDT', 'APTUSDT',
        'AVAXUSDT', 'MATICUSDT', 'ATOMUSDT', 'XLMUSDT', 'RNDRUSDT'
    ]
    ...
    def fetch_crypto_ohlc(coin, interval='1h', start_str='1 Jan, 2020'):
        ...
        while start_time < end_time:
            ...
            params = {
                ...
                'endTime': int((start_time + timedelta(days=30)).timestamp() * 1000)  # Fetching 30 days at a time
            }
        ...
        ohlc_df = pd.DataFrame(all_data, columns=[
        ...
        ohlc_df['Date'] = ohlc_df['Date'].dt.tz_localize('UTC').dt.tz_convert('Europe/Berlin')
        ohlc_df.set_index('Date', inplace=True)

        return ohlc_df[['Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume', 'Ticker', 'Ticker_Type']]
    ...
    cols = ['Year', 'Month', 'Weekday', 'Hour', 'Ticker', 'Ticker_Type'] + [col for col in all_data.columns if col not in ['Year', 'Month', 'Weekday', 'Hour', 'Ticker', 'Ticker_Type']]
