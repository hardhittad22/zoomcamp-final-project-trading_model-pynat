...
# Print total number of investment opportunities and days
    ...
    new_df['sim1_gross_rev_pred14'] = new_df['Trade_Signal1'] * 150 * (
...
# Strategy2 enerate trade signal based on selected features (as indicated by the random forest model)
    ...
    # Calculate gross revenue with clipping to account for stop loss and take profit limits
    new_df['sim2_gross_rev_pred14'] = new_df['Trade_Signal2'] * 150 * (
    new_df['Growth_Future_1h'].clip(lower=(1 + stop_loss_threshold), upper=(1 + take_profit_threshold)) - 1
    )
    # Calculate trading fees (assumed 0.2% per trade)
    ...
    new_df['sim3_gross_rev_pred14'] = new_df['Trade_Signal3'] * 150 * (
    ...
    new_df['sim4_gross_rev_pred14'] = new_df['Trade_Signal4'] * 150 * (
