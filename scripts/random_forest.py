...
def fit_random_forest(X_train, y_train, X_valid, y_valid, X_test, y_test):
    ...
    print(f'The best precision is {best_precision} and the best depth is {best_depth} and best estimators = {best_estimators}')

    # Train the final model with the best parameters
    rf_model = RandomForestClassifier(max_depth=best_depth, n_estimators=best_estimators, random_state=42)
...
def analyze_rf_model(new_df, rf_model, X_all, X_test, y_test):
    ...
    df_scores.plot.line(x='threshold', y=['precision', 'recall', 'f1_score'], 
...
def train_random_forest_model():
    ...
    X_train_valid = new_df[new_df.Split.isin(['Train', 'Validation'])].drop(columns=[TO_PREDICT])
    ...
    rf_model, best_depth, best_estimators = fit_random_forest(X_train=X_train_valid, y_train=y_train_valid,
