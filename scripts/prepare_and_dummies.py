...
def add_black_swan_events(df):
    ...
    black_swan_events = [
        ...
        {
            "Event": "Federal Crypto Regulations due to Russia Sanctions",
            "Date": "2022-03-02",
        },
...
def prepare():
    ...
    # Converte Date
    df_with_dummies["Date"] = pd.to_datetime(
        df_with_dummies["Date"], utc=True
    ...
    if "RSI_14" in df_with_dummies.columns:
        df_with_dummies["RSI_binned"] = pd.cut(
            df_with_dummies["RSI_14"],
            bins=[0, 30, 70, 100],
    ...
    if (
        "MACD_12_26_9" in df_with_dummies.columns
        and "MACDs_12_26_9" in df_with_dummies.columns
        and "MACDh_12_26_9" in df_with_dummies.columns
    ):
        df_with_dummies["MACD_12_26_9"] = df_with_dummies["MACD_12_26_9"].astype(
            "float32"
        )
        df_with_dummies["MACDs_12_26_9"] = df_with_dummies["MACDs_12_26_9"].astype(
            "float32"
        )
        df_with_dummies["MACDh_12_26_9"] = df_with_dummies["MACDh_12_26_9"].astype(
            "float32"
        )
        (
            df_with_dummies["MACD"],
            df_with_dummies["MACD_signal"],
            df_with_dummies["MACD_hist"],
        ) = (
            df_with_dummies["MACD_12_26_9"],
            df_with_dummies["MACDs_12_26_9"],
            df_with_dummies["MACDh_12_26_9"],
        )
        df_with_dummies["MACD_binned"] = pd.cut(
            df_with_dummies["MACD"],
    ...
    if "ADX_14" in df_with_dummies.columns:
        ...
        df_with_dummies["ADX_binned"] = pd.cut(
            df_with_dummies["ADX_14"],
            bins=[0, 25, 50, 75, 100],
    ...
    if "Volume" in df_with_dummies.columns:
        ...
        df_with_dummies["Volume_binned"] = pd.cut(
            df_with_dummies["Volume"],
            bins=[
                0,
                df_with_dummies["Volume"].quantile(0.33),
                df_with_dummies["Volume"].quantile(0.66),
                np.inf,
            ],
    ...
    if "AROONOSC_14" in df_with_dummies.columns:
        df_with_dummies["AROONOSC_14"] = df_with_dummies["AROONOSC_14"].astype(
            "float32"
        )
        df_with_dummies["Aroon_binned"] = pd.cut(
            df_with_dummies["AROONOSC_14"],
            bins=[-100, -50, 50, 100],
    ...
    if "BBP_5_2.0" in df_with_dummies.columns:
        ...
        df_with_dummies["BBP_binned"] = pd.cut(
            df_with_dummies["BBP_5_2.0"],
            bins=[0, 0.2, 0.8, 1],
            labels=["low", "medium", "high"],
        )
    else:
    ...
    if "Close" in df_with_dummies.columns:
        ...
        df_with_dummies["SMA_Cross"] = (
            df_with_dummies["SMA_50"] - df_with_dummies["SMA_200"]
        )
    else:
        print("Close column not found in DataFrame")
    ...
    new_dummy_variables = pd.get_dummies(
        df_with_dummies[new_categorical], dtype="int32"
    )

    NUMERICAL.extend(["Pct_Change", "SMA_50", "SMA_200", "SMA_Cross"])
